...
<div class="container mx-auto px-6 pb-4">
    ...
    <h2 class="text-2xl font-semibold mb-4">Edit Cash Book Entry</h2>

    <form action="{{ route('cash-book.update', $entry->id) }}" method="POST" class="bg-white p-8 rounded-lg shadow-md">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
                ...
                <select name="transaction_type" id="transaction_type" required class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
                    <option value="receipt" {{ old('transaction_type', $entry->transaction_type) == 'receipt' ? 'selected' : '' }}>Receipt (Money In)</option>
                    <option value="payment" {{ old('transaction_type', $entry->transaction_type) == 'payment' ? 'selected' : '' }}>Payment (Money Out)</option>
                </select>
            ...
            <div>
                ...
                <input type="text" name="reference" id="reference" value="{{ old('reference', $entry->reference) }}" class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
                @error('reference')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div>
                ...
                <select name="related_invoice_id" id="related_invoice_id" class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
                    ...
                    @foreach($invoices as $invoice)
                        <option value="{{ $invoice->id }}" {{ old('related_invoice_id', $entry->related_invoice_id) == $invoice->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <select name="related_client_id" id="related_client_id" class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
                    ...
                    @foreach($clients as $client)
                        <option value="{{ $client->id }}" {{ old('related_client_id', $entry->related_client_id) == $client->id ? 'selected' : '' }}>
                            {{ $client->client_name }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="flex justify-end gap-4">
            <a href="{{ route('cash-book.show', $entry->id) }}" class="bg-gray-500 text-white px-6 py-3 rounded-md shadow-md hover:bg-gray-600 transition">Cancel</a>
