...
<div class="mx-auto max-w-7xl px-3 pb-4 sm:px-5">
    ...
    <div class="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
        <h3 class="font-semibold text-blue-800 mb-2">Accounts Payable (AP) Ledger Description</h3>
        <p class="text-sm text-blue-700">
            Tracks money the company owes to suppliers and vendors. Records outstanding bills, payment due dates, and payment status. Helps manage payment schedules and avoid late payment penalties. Essential for maintaining good supplier relationships and cash flow planning.
        </p>
    </div>

    <h2 class="text-2xl font-semibold mb-4">AP Ledger Entries</h2>

    <!-- Summary Cards -->
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div class="bg-red-50 p-4 rounded-lg border border-red-200">
            <h3 class="text-sm font-medium text-red-700">Total Payable</h3>
            <p class="text-2xl font-bold text-red-600">₦{{ number_format($totalPayable, 2) }}</p>
        </div>
        <div class="bg-green-50 p-4 rounded-lg border border-green-200">
            <h3 class="text-sm font-medium text-green-700">Paid Amount</h3>
            <p class="text-2xl font-bold text-green-600">₦{{ number_format($paidAmount, 2) }}</p>
        </div>
        <div class="bg-orange-50 p-4 rounded-lg border border-orange-200">
            <h3 class="text-sm font-medium text-orange-700">Outstanding</h3>
            <p class="text-2xl font-bold text-orange-600">₦{{ number_format($outstandingBalance, 2) }}</p>
        </div>
        <div class="bg-yellow-50 p-4 rounded-lg border border-yellow-200">
            <h3 class="text-sm font-medium text-yellow-700">Overdue</h3>
            <p class="text-2xl font-bold text-yellow-600">₦{{ number_format($overdueAmount, 2) }}</p>
        </div>
    </div>

    <!-- Search and Filters -->
    <div class="mb-6 flex flex-col gap-4 lg:flex-row lg:flex-wrap lg:items-start lg:justify-between lg:gap-4">
        <form action="{{ route('ap-ledger.index') }}" method="GET" class="flex w-full min-w-0 flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-stretch">
            ...
            <a href="{{ route('ap-ledger.index') }}" class="bg-gray-500 text-white px-4 py-3 rounded-md shadow-md hover:bg-gray-600 transition">Clear</a>
        </form>
        <div class="flex w-full flex-shrink-0 flex-col gap-2 sm:w-auto sm:flex-row sm:flex-wrap">
            <a href="{{ route('ap-ledger.export', request()->query()) }}" class="bg-green-600 text-white px-4 py-3 rounded-md shadow-md hover:bg-green-700 transition">Export CSV</a>
            <a href="{{ route('ap-ledger.create') }}" class="bg-red-600 text-white px-4 py-3 rounded-md shadow-md hover:bg-red-700 transition">Add Entry</a>
        </div>
    </div>
    ...
    <div class="-mx-3 overflow-x-auto sm:mx-0">
        <table class="min-w-[44rem] w-full bg-white border border-gray-200 text-sm shadow-md rounded-lg sm:text-base">
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50 {{ $entry->isOverdue() ? 'bg-red-50' : '' }}">
                    <td class="px-6 py-4">{{ $entry->entry_date->format('M d, Y') }}</td>
                    <td class="px-6 py-4">{{ $entry->supplier_name }}</td>
                    <td class="px-6 py-4">{{ $entry->invoice_number ?? 'N/A' }}</td>
                    <td class="px-6 py-4">₦{{ number_format($entry->amount, 2) }}</td>
                    <td class="px-6 py-4 {{ $entry->isOverdue() ? 'text-red-600 font-bold' : '' }}">{{ $entry->due_date->format('M d, Y') }}</td>
                    ...
                    <td class="px-6 py-4">
                        <div class="flex flex-col gap-1">
                            <a href="{{ route('ap-ledger.show', $entry->id) }}" class="bg-blue-500 text-white px-3 py-1 rounded text-sm hover:bg-blue-600 transition">View</a>
                            <a href="{{ route('ap-ledger.edit', $entry->id) }}" class="bg-yellow-500 text-white px-3 py-1 rounded text-sm hover:bg-yellow-600 transition">Edit</a>
                            @if(auth()->user()->hasRole('super-admin'))
                            <form action="{{ route('ap-ledger.destroy', $entry->id) }}" method="POST" class="inline-block">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                <tr>
                    <td colspan="8" class="px-6 py-4 text-center text-gray-500">No entries found.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
