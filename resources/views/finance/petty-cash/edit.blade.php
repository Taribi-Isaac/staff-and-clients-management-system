...
<div class="container mx-auto px-6 pb-4">
    ...
    <h2 class="text-2xl font-semibold mb-4">Edit Petty Cash Entry</h2>

    <form action="{{ route('petty-cash.update', $entry->id) }}" method="POST" class="bg-white p-8 rounded-lg shadow-md">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
                ...
                <select name="transaction_type" id="transaction_type" required class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
                    <option value="expense" {{ old('transaction_type', $entry->transaction_type) == 'expense' ? 'selected' : '' }}>Expense</option>
                    <option value="replenishment" {{ old('transaction_type', $entry->transaction_type) == 'replenishment' ? 'selected' : '' }}>Replenishment</option>
                </select>
            ...
            <div>
                ...
                <input type="number" name="amount" id="amount" value="{{ old('amount', $entry->amount) }}" step="0.01" min="0.01" required class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
                @error('amount')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="text" name="receiver_beneficiary" id="receiver_beneficiary" value="{{ old('receiver_beneficiary', $entry->receiver_beneficiary) }}" class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500" placeholder="Name of person who received payment">
            ...
            <div>
                ...
                <input type="text" name="category" id="category" value="{{ old('category', $entry->category) }}" class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
                @error('category')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="text" name="receipt_number" id="receipt_number" value="{{ old('receipt_number', $entry->receipt_number) }}" class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
            ...
            <div>
                ...
                <input type="text" name="approved_by" id="approved_by" value="{{ old('approved_by', $entry->approved_by) }}" class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
                @error('approved_by')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div>
                ...
                <select name="approved_by_user_id" id="approved_by_user_id" class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
                    ...
                    @foreach($users as $user)
                        <option value="{{ $user->id }}" {{ old('approved_by_user_id', $entry->approved_by_user_id) == $user->id ? 'selected' : '' }}>
                            {{ $user->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="md:col-span-2">
                ...
                <input type="text" name="description" id="description" value="{{ old('description', $entry->description) }}" required class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
                @error('description')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
        ...
        <div class="flex justify-end gap-4">
            <a href="{{ route('petty-cash.show', $entry->id) }}" class="bg-gray-500 text-white px-6 py-3 rounded-md shadow-md hover:bg-gray-600 transition">Cancel</a>
