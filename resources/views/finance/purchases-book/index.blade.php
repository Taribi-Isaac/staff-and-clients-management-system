...
<div class="mx-auto max-w-7xl px-3 pb-4 sm:px-5">
    ...
    <div class="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
        <h3 class="font-semibold text-blue-800 mb-2">Purchases Book Description</h3>
        <p class="text-sm text-blue-700">
            Records all purchases made by the company from suppliers. Tracks expenses for inventory, equipment, services, and other business purchases. Helps monitor spending and manage supplier relationships. Essential for expense tracking and cost management.
        </p>
    </div>

    <h2 class="text-2xl font-semibold mb-4">Purchases Book Entries</h2>

    <!-- Summary Cards -->
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div class="bg-red-50 p-4 rounded-lg border border-red-200">
            <h3 class="text-sm font-medium text-red-700">Total Purchases</h3>
            <p class="text-2xl font-bold text-red-600">₦{{ number_format($totalPurchases, 2) }}</p>
        </div>
        <div class="bg-blue-50 p-4 rounded-lg border border-blue-200">
            <h3 class="text-sm font-medium text-blue-700">Paid Purchases</h3>
            <p class="text-2xl font-bold text-blue-600">₦{{ number_format($paidPurchases, 2) }}</p>
        </div>
        <div class="bg-orange-50 p-4 rounded-lg border border-orange-200">
            <h3 class="text-sm font-medium text-orange-700">Pending Purchases</h3>
            <p class="text-2xl font-bold text-orange-600">₦{{ number_format($pendingPurchases, 2) }}</p>
        </div>
    </div>

    <!-- Search and Filters -->
    <div class="mb-6 flex flex-col gap-4 lg:flex-row lg:flex-wrap lg:items-start lg:justify-between lg:gap-4">
        <form action="{{ route('purchases-book.index') }}" method="GET" class="flex w-full min-w-0 flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-stretch">
            ...
            <a href="{{ route('purchases-book.index') }}" class="bg-gray-500 text-white px-4 py-3 rounded-md shadow-md hover:bg-gray-600 transition">Clear</a>
        </form>
        <div class="flex w-full flex-shrink-0 flex-col gap-2 sm:w-auto sm:flex-row sm:flex-wrap">
            <a href="{{ route('purchases-book.export', request()->query()) }}" class="bg-green-600 text-white px-4 py-3 rounded-md shadow-md hover:bg-green-700 transition">Export CSV</a>
            <a href="{{ route('purchases-book.create') }}" class="bg-red-600 text-white px-4 py-3 rounded-md shadow-md hover:bg-red-700 transition">Add Entry</a>
        </div>
    </div>
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex flex-col gap-1">
                            <a href="{{ route('purchases-book.show', $entry->id) }}" class="bg-blue-500 text-white px-3 py-1 rounded text-sm hover:bg-blue-600 transition">View</a>
                            <a href="{{ route('purchases-book.edit', $entry->id) }}" class="bg-yellow-500 text-white px-3 py-1 rounded text-sm hover:bg-yellow-600 transition">Edit</a>
                            @if(auth()->user()->hasRole('super-admin'))
                            <form action="{{ route('purchases-book.destroy', $entry->id) }}" method="POST" class="inline-block">
                                @csrf
                                @method('DELETE')
