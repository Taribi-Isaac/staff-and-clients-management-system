...
<div class="mx-auto max-w-7xl px-3 pb-4 sm:px-5">
    ...
    <div class="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
        <h3 class="font-semibold text-blue-800 mb-2">Payroll Book Description</h3>
        <p class="text-sm text-blue-700">
            Records all employee salary and wage payments. Tracks basic salary, allowances, deductions (tax, pension, etc.), and net pay. Helps manage payroll expenses and ensure timely employee payments. Essential for payroll management and compliance with labor regulations.
        </p>
    </div>

    <h2 class="text-2xl font-semibold mb-4">Payroll Book Entries</h2>

    <!-- Summary Cards -->
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div class="bg-purple-50 p-4 rounded-lg border border-purple-200">
            <h3 class="text-sm font-medium text-purple-700">Total Payroll</h3>
            <p class="text-2xl font-bold text-purple-600">₦{{ number_format($totalPayroll, 2) }}</p>
        </div>
        <div class="bg-green-50 p-4 rounded-lg border border-green-200">
            <h3 class="text-sm font-medium text-green-700">Paid Payroll</h3>
            <p class="text-2xl font-bold text-green-600">₦{{ number_format($paidPayroll, 2) }}</p>
        </div>
        <div class="bg-orange-50 p-4 rounded-lg border border-orange-200">
            <h3 class="text-sm font-medium text-orange-700">Pending Payroll</h3>
            <p class="text-2xl font-bold text-orange-600">₦{{ number_format($pendingPayroll, 2) }}</p>
        </div>
    </div>

    <!-- Search and Filters -->
    <div class="mb-6 flex flex-col gap-4 lg:flex-row lg:flex-wrap lg:items-start lg:justify-between lg:gap-4">
        <form action="{{ route('payroll-book.index') }}" method="GET" class="flex w-full min-w-0 flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-stretch">
            ...
            <a href="{{ route('payroll-book.index') }}" class="bg-gray-500 text-white px-4 py-3 rounded-md shadow-md hover:bg-gray-600 transition">Clear</a>
        </form>
        <div class="flex w-full flex-shrink-0 flex-col gap-2 sm:w-auto sm:flex-row sm:flex-wrap">
            <a href="{{ route('payroll-book.export', request()->query()) }}" class="bg-green-600 text-white px-4 py-3 rounded-md shadow-md hover:bg-green-700 transition">Export CSV</a>
            <a href="{{ route('payroll-book.create') }}" class="bg-red-600 text-white px-4 py-3 rounded-md shadow-md hover:bg-red-700 transition">Add Entry</a>
        </div>
    </div>
        ...
        <table class="min-w-[44rem] w-full bg-white border border-gray-200 text-sm shadow-md rounded-lg sm:text-base">
            <thead class="bg-red-600 text-white">
                <tr>
                    ...
                    <th class="px-6 py-4 text-left font-semibold">Pay Period</th>
                    <th class="px-6 py-4 text-left font-semibold">Basic Salary</th>
                    <th class="px-6 py-4 text-left font-semibold">Net Pay</th>
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4">{{ $entry->employee_name }}</td>
                    <td class="px-6 py-4">{{ $entry->pay_period_start->format('M d') }} - {{ $entry->pay_period_end->format('M d, Y') }}</td>
                    <td class="px-6 py-4">₦{{ number_format($entry->basic_salary, 2) }}</td>
                    <td class="px-6 py-4 font-semibold">₦{{ number_format($entry->net_pay, 2) }}</td>
                    <td class="px-6 py-4">
                        <span class="px-2 py-1 rounded text-xs {{ $entry->payment_status === 'paid' ? 'bg-green-100 text-green-800' : ($entry->payment_status === 'failed' ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800') }}">
                            {{ $entry->payment_status_label }}
                        </span>
                    </td>
                    <td class="px-6 py-4">
                        <div class="flex flex-col gap-1">
                            <a href="{{ route('payroll-book.show', $entry->id) }}" class="bg-blue-500 text-white px-3 py-1 rounded text-sm hover:bg-blue-600 transition">View</a>
                            <a href="{{ route('payroll-book.edit', $entry->id) }}" class="bg-yellow-500 text-white px-3 py-1 rounded text-sm hover:bg-yellow-600 transition">Edit</a>
                            @if(auth()->user()->hasRole('super-admin'))
                            <form action="{{ route('payroll-book.destroy', $entry->id) }}" method="POST" class="inline-block">
                                @csrf
                                @method('DELETE')
