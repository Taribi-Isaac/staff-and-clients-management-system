...
<div class="container mx-auto px-6 pb-4">
    ...
    <h2 class="text-2xl font-semibold mb-4">Payroll Book Entry Details</h2>
    ...
    <div class="bg-white p-8 rounded-lg shadow-md mb-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
                ...
                <p class="text-lg">{{ $entry->employee_name }}</p>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Pay Period</label>
                <p class="text-lg">{{ $entry->pay_period_start->format('M d, Y') }} - {{ $entry->pay_period_end->format('M d, Y') }}</p>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Basic Salary</label>
                <p class="text-lg">₦{{ number_format($entry->basic_salary, 2) }}</p>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Allowances</label>
                <p class="text-lg">₦{{ number_format($entry->allowances, 2) }}</p>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Deductions</label>
                <p class="text-lg">₦{{ number_format($entry->deductions, 2) }}</p>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Net Pay</label>
                <p class="text-lg font-semibold text-red-600">₦{{ number_format($entry->net_pay, 2) }}</p>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Payment Method</label>
                <p class="text-lg">{{ $entry->payment_method_label }}</p>
            </div>
            @if($entry->bank_name)
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Bank Name</label>
                <p class="text-lg">{{ $entry->bank_name }}</p>
            </div>
            @endif
            @if($entry->account_number)
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Account Number</label>
                <p class="text-lg">{{ $entry->account_number }}</p>
            </div>
            @endif
            <div>
            ...
            <div>
                ...
                <p class="text-lg">{{ $entry->payment_date->format('M d, Y') }}</p>
            </div>
            @endif
            <div>
    ...
    <div class="flex gap-4">
        <a href="{{ route('payroll-book.edit', $entry->id) }}" class="bg-yellow-500 text-white px-4 py-2 rounded-md shadow-md hover:bg-yellow-600 transition">Edit Entry</a>
        <a href="{{ route('payroll-book.index') }}" class="bg-gray-500 text-white px-4 py-2 rounded-md shadow-md hover:bg-gray-600 transition">Back to Payroll Book</a>
    </div>
</div>
@endsection
