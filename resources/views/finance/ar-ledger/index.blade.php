...
<div class="mx-auto max-w-7xl px-3 pb-4 sm:px-5">
    ...
    <div class="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
        <h3 class="font-semibold text-blue-800 mb-2">Accounts Receivable (AR) Ledger Description</h3>
        <p class="text-sm text-blue-700">
            Tracks money owed to the company by clients/customers. Records outstanding invoices, payment due dates, and payment status. Helps manage collections and identify overdue accounts. Essential for cash flow management and ensuring timely payment collection.
        </p>
    </div>

    <h2 class="text-2xl font-semibold mb-4">AR Ledger Entries</h2>

    <!-- Summary Cards -->
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div class="bg-blue-50 p-4 rounded-lg border border-blue-200">
            <h3 class="text-sm font-medium text-blue-700">Total Receivable</h3>
            <p class="text-2xl font-bold text-blue-600">₦{{ number_format($totalReceivable, 2) }}</p>
        </div>
        ...
        <div class="bg-red-50 p-4 rounded-lg border border-red-200">
            <h3 class="text-sm font-medium text-red-700">Overdue</h3>
            <p class="text-2xl font-bold text-red-600">₦{{ number_format($overdueAmount, 2) }}</p>
        </div>
    </div>

    <!-- Search and Filters -->
    <div class="mb-6 flex flex-col gap-4 lg:flex-row lg:flex-wrap lg:items-start lg:justify-between lg:gap-4">
        <form action="{{ route('ar-ledger.index') }}" method="GET" class="flex w-full min-w-0 flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-stretch">
            ...
            <a href="{{ route('ar-ledger.index') }}" class="bg-gray-500 text-white px-4 py-3 rounded-md shadow-md hover:bg-gray-600 transition">Clear</a>
        </form>
        <div class="flex w-full flex-shrink-0 flex-col gap-2 sm:w-auto sm:flex-row sm:flex-wrap">
            <a href="{{ route('ar-ledger.export', request()->query()) }}" class="bg-green-600 text-white px-4 py-3 rounded-md shadow-md hover:bg-green-700 transition">Export CSV</a>
            <a href="{{ route('ar-ledger.create') }}" class="bg-red-600 text-white px-4 py-3 rounded-md shadow-md hover:bg-red-700 transition">Add Entry</a>
        </div>
    </div>
                ...
                <tr class="hover:bg-gray-50 {{ $entry->isOverdue() ? 'bg-red-50' : '' }}">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex flex-col gap-1">
                            <a href="{{ route('ar-ledger.show', $entry->id) }}" class="bg-blue-500 text-white px-3 py-1 rounded text-sm hover:bg-blue-600 transition">View</a>
                            <a href="{{ route('ar-ledger.edit', $entry->id) }}" class="bg-yellow-500 text-white px-3 py-1 rounded text-sm hover:bg-yellow-600 transition">Edit</a>
                            @if(auth()->user()->hasRole('super-admin'))
                            <form action="{{ route('ar-ledger.destroy', $entry->id) }}" method="POST" class="inline-block">
                                @csrf
                                @method('DELETE')
