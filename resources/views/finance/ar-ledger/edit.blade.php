...
<div class="container mx-auto px-6 pb-4">
    ...
    <h2 class="text-2xl font-semibold mb-4">Edit AR Ledger Entry</h2>

    <form action="{{ route('ar-ledger.update', $entry->id) }}" method="POST" class="bg-white p-8 rounded-lg shadow-md">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
                ...
                <input type="text" name="client_name" id="client_name" value="{{ old('client_name', $entry->client_name) }}" required class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
                @error('client_name')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div>
                ...
                <select name="invoice_id" id="invoice_id" class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
                    ...
                    @foreach($invoices as $invoice)
                        <option value="{{ $invoice->id }}" {{ old('invoice_id', $entry->invoice_id) == $invoice->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <input type="date" name="due_date" id="due_date" value="{{ old('due_date', $entry->due_date->format('Y-m-d')) }}" required class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
                @error('due_date')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div>
                ...
                <select name="status" id="status" required class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
                    <option value="pending" {{ old('status', $entry->status) == 'pending' ? 'selected' : '' }}>Pending</option>
                    <option value="partial" {{ old('status', $entry->status) == 'partial' ? 'selected' : '' }}>Partial</option>
                    <option value="paid" {{ old('status', $entry->status) == 'paid' ? 'selected' : '' }}>Paid</option>
                    <option value="overdue" {{ old('status', $entry->status) == 'overdue' ? 'selected' : '' }}>Overdue</option>
                    <option value="written_off" {{ old('status', $entry->status) == 'written_off' ? 'selected' : '' }}>Written Off</option>
                </select>
                @error('status')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            ...
            <div>
                ...
                <input type="number" name="balance" id="balance" value="{{ old('balance', $entry->balance) }}" step="0.01" min="0" required class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
                @error('balance')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
        ...
        <div class="flex justify-end gap-4">
            <a href="{{ route('ar-ledger.show', $entry->id) }}" class="bg-gray-500 text-white px-6 py-3 rounded-md shadow-md hover:bg-gray-600 transition">Cancel</a>
