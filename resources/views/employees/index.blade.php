...
<div class="container mx-auto px-6 pb-4">
    <h1 class="text-3xl font-bold mb-6 text-center">All Employee</h1>
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full bg-white border border-gray-200 shadow-md rounded-lg">
            ...
            <tbody class="divide-y divide-gray-200">
                @foreach($employees as $index => $member)
    ...
    <img src="{{ $member->passport ? asset('storage/passports/' . $member->passport) : asset('images/user.jpg') }}" 
         ...
         class="h-16 w-16 rounded-full object-cover">
                    ...
                    <td class="px-6 py-4 flex flex-col justify-between items-center space-x-2">
                        <a href="{{ route('employees.show', $member->id) }}" class="bg-blue-500 text-white px-4 py-2 mb-1 rounded-md shadow-md hover:bg-blue-600 transition">View</a>

                        <a href="{{ route('employees.edit', $member->id) }}" class="bg-yellow-500 text-white px-4 py-2 mb-1 rounded-md shadow-md hover:bg-yellow-600 transition">Update</a>

                        <form action="{{ route('employees.destroy', $member->id) }}" method="POST" class="inline-block">
                            @csrf
                            @method('DELETE')
        ...
        @if($employees->isEmpty())
            <p class="text-center text-gray-500 mt-4">No employees found.</p>
        @endif
    </div>
